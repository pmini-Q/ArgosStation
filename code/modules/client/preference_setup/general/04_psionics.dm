...
/datum/category_item/player_setup_item/physical/psionics/sanitize_character(var/savefile/S)
	...
	if(pref.psionic)
		pref.psi_faculty = sanitize_inlist(pref.psi_faculty, PSIONIC_FACULTIES_NUMERIC, get_key_by_index(PSIONIC_FACULTIES_NUMERIC, 0))
		pref.psi_rank = sanitize_inlist(pref.psi_rank, PSIONIC_RANKS_NUMERIC, get_key_by_index(PSIONIC_RANKS_NUMERIC, 0))
		pref.psi_implant = sanitize_bool(pref.psi_implant, FALSE)
	else
		pref.psi_faculty = sanitize_bool(pref.psi_faculty, FALSE)
		pref.psi_rank = sanitize_bool(pref.psi_rank, FALSE)
		pref.psi_implant = sanitize_bool(pref.psi_implant, FALSE)
...
/datum/category_item/player_setup_item/physical/psionics/content()
	...
	if((config.usealienwhitelist && whitelist_lookup("psionics", pref.client_ckey)) | !config.usealienwhitelist)
		...
		if(pref.psionic)
			...
			. += "<a href='?src=\ref[src];psi_faculty=1'>[psiFaculty]</a><br>"
			...
			. += "Implant: "
			. += "<a href='?src=\ref[src];psi_implant=1'>[pref.psi_implant ? "Yes" : "No"]</a>"
			if(!pref.psi_implant)
				. += "<br>"
...
/datum/category_item/player_setup_item/physical/psionics/OnTopic(var/href, var/list/href_list, var/mob/user)
	if(href_list["psionic"])
		...
		if(!pref.psionic)
			pref.psi_faculty = "None"
			pref.psi_rank = PSI_RANK_BLUNT
			pref.psi_implant = FALSE
		else
			pref.psi_faculty = PSI_COERCION
			pref.psi_rank = PSI_RANK_LATENT
			pref.psi_implant = TRUE
		return TOPIC_REFRESH

	else if(href_list["psi_faculty"])
		var/switchFaculty = input("Select a psionic faculty:", "Psionic Faculty", pref.psi_faculty) as anything in PSIONIC_FACULTIES
		if(!switchFaculty)
		...
		switch(lowertext(switchFaculty))
			if("coercion")
				pref.psi_faculty = PSI_COERCION
			if("energistics")
				pref.psi_faculty = PSI_ENERGISTICS
			if("psychokinesis")
				pref.psi_faculty = PSI_PSYCHOKINESIS
			if("redaction")
				pref.psi_faculty = PSI_REDACTION
		return TOPIC_REFRESH
	...
	else if(href_list["psi_implant"])
		pref.psi_implant = !pref.psi_implant
		return TOPIC_REFRESH